        ...
        return (
          <div className="w-24 h-16 bg-black rounded-[24px] shadow-xl flex items-center justify-center transform transition-transform duration-300 hover:scale-105">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 2L14.8 7.6L21 8.4L16.5 12.8L17.8 19L12 16L6.2 19L7.5 12.8L3 8.4L9.2 7.6L12 2Z" fill="#F7E353"/>
            </svg>
          </div>
        )
      case "diamond":
    ...
    <section className="container mx-auto px-4 py-24 relative z-20">
      ...
      >
        ...
        <p className="text-muted">
          Roadmap description lorem ipsum.
        ...
        <div className="relative">
          {roadmapItems.map((item, index) => (
            <motion.div
              ...
              className={`
                flex items-center
                ${index === 0 ? 'h-[120px]' : index === 1 ? 'h-[180px]' : 'h-[120px]'}
                ${item.isLeft ? 'flex-row' : 'flex-row-reverse'}
                justify-center gap-6 md:gap-8
              `}
            >
              <div className={`flex-1 ${item.isLeft ? 'text-right' : 'text-left'}`}>
                <motion.div
                  whileHover={{ scale: 1.02 }}
                  className={`inline-block ${item.isLeft ? 'ml-auto' : 'mr-auto'}`}
                >
